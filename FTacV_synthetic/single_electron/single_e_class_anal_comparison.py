...
class single_electron:
    ...
    def kaiser_filter(self, time_series, harmonical=False):
        ...
        #    labels[i]=str(self.harmonic_range[i])+"f"
            ...
            #filter_bit=np.multiply(filter_bit, np.kaiser(len(filter_bit), 6))
            filter_bit_inverse=top_hat[np.where((frequencies<(true_harm+(self.nd_param.omega*self.filter_val))) & (frequencies>true_harm-(self.nd_param.omega*self.filter_val)))]
            #filter_bit_inverse=np.multiply(filter_bit, np.kaiser(len(filter_bit), 6))
            ...
            inverse_results[np.where((frequencies<(true_harm+(self.nd_param.omega*self.filter_val))) & (frequencies>true_harm-(self.nd_param.omega*self.filter_val)))]=filter_bit_inverse
        #print labels
        #print xlocs
        ...
        if harmonical==True:
            return harmonics
        last_harm=(self.harmonic_range[-1]*self.nd_param.omega*1.029)+(self.nd_param.omega*self.filter_val)
        last_point=np.where(frequencies<last_harm)
        #print last_point
        results=results[:last_point[0][-1]]
        inverse_results=inverse_results[:last_point[0][-1]]
        #top_hat=np.append((results), (np.flip(inverse_results)))
    ...
    def simulate(self,parameters, frequencies, flag='optimise', flag2='fourier', test="yes"):
        ...
        time_series=isolver.e_surface(self.nd_param.Cdl,self.nd_param.CdlE1,self.nd_param.CdlE2,self.nd_param.CdlE3,self.nd_param.omega,self.nd_param.v, self.nd_param.alpha , \
                                    self.nd_param.E_start,  self.nd_param.E_reverse,  self.nd_param.d_E,  self.nd_param.Ru,1600, self.time_vec,  self.nd_param.gamma, \
