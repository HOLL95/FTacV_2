...
double et(double E_start, double E_reverse, double omega, double phase, double rand_val, double t){
	double amp=abs(E_start-E_reverse)/2;
  double rand=rand() %-rand_val*amp + (2*rand_val*amp);
  double random=rand() %
	double E_t=(E_start+amp)+amp*(std::sin((omega*t)+phase))+rand;

	return E_t;
}

double dEdt(double E_start, double E_reverse, double omega, double phase, double t){
	double  amp=abs(E_start-E_reverse)/2;
  double dedt=(amp*omega*std::cos(omega*t+phase));
	return dedt;
}
...
py::object e_surface(double Cdl, double CdlE, double CdlE2, double CdlE3,double omega,double v,double alpha ,double E_start, double E_reverse, \
  ...
  if (times.size()==0) {
    ...
    const double E = et(E_start, E_reverse, omega, phase, rand_val,  t1+dt);
