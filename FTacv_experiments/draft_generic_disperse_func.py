def generic_dispersion(self, solver):
    ...
    for i in range(0, len(sim_params)):
        ...
        if sim_params[i]=="k0":
            sim_params[i]="k_0"
    start=time.time()
    for i in range(0, len(value_combinations)):
        for j in range(0, len(self.simulation_options["dispersion_parameters"])):
            self.nd_param_dict[sim_params[j]]=value_combinations[i][j]
        time_series_current=solver(self.nd_param_dict, self.time_vec,self.simulation_options["method"], -1, self.bounds_val)
        time_series=np.add(time_series, np.multiply(time_series_current, np.prod(weight_combinations[i])))
    return time_series
