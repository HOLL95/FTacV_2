...
param_names=["Noramp_3_cv_fixed_ru.fixed_alpha", "Ramped_3_cv_high_ru.ts"]
...
ramp_harm=harmonics(list(range(2, 6)), ramped_class.nd_param.omega, 0.03)
master_optim_list=["E0_mean", "E0_std","k_0","Ru","Cdl","CdlE1", "CdlE2","gamma","cap_phase","phase","alpha"]
class_list=[noramp_ts_class, ramped_class]
...
plt.plot(ramped_voltage, ramped_current, label="Ramped data")#, color="orange")

plt.xlabel("Time(s)")
plt.ylabel("Current(A)")
print(param_list[0][1], param_list[0][0])

plt.axvline(param_list[0][0],color="red",linestyle="--", label="Noramp $E_0$")
plt.axvline(param_list[1][0], color="green", linestyle="--", label="Ramped $E_0$")

def normalise(val, min, max):
    print((val-min)/(min-max))
    return (val-min)/(max-min)
ax=plt.gca()
xmin, xmax=ax.get_xlim()
ax.axhline(1.05*max(ramped_current),normalise(param_list[1][0]-param_list[1][1], xmin, xmax), normalise(param_list[1][0]+param_list[1][1],xmin, xmax), color="green", alpha=0.5)
ax.axhline(0.85*max(noramp_current),normalise(param_list[0][0]-param_list[0][1],xmin, xmax), normalise(param_list[0][0]+param_list[0][1],xmin, xmax), color="red", alpha=0.5)
...
delta=1e-3
e0=param_list[0][0]
e_std=param_list[0][1]
half_1=tuple(np.where(ramped_time<(ramped_time[-1]/2)))
half_2=tuple(np.where(ramped_time>(ramped_time[-1]/2)))
ramped_voltage1=ramped_voltage[half_1]
ramped_voltage2=ramped_voltage[half_2]

volts_val_1=np.where((ramped_voltage1>e0-(e_std)) & (ramped_voltage1<e0+(e_std)))
volts_val_2=np.where((ramped_voltage2>e0-(e_std)) & (ramped_voltage2<e0+(e_std)))
plt.axvspan(ramped_time[volts_val_1[0][0]], ramped_time[volts_val_1[0][-1]], color="red", linestyle="--", alpha=0.2)
plt.axvspan(ramped_time[volts_val_2[0][0]+len(ramped_voltage1)], ramped_time[volts_val_2[0][-1]+len(ramped_voltage1)], color="red", linestyle="--", alpha=0.2)
plt.axvline(ramped_time[volts_val_1[0][int(len(volts_val_1[0])/2)]], color="red", linestyle="--", label="Noramp $E_0$")
plt.axvline(ramped_time[volts_val_2[0][int(len(volts_val_2[0])/2)]+len(ramped_voltage1)],color="red", linestyle="--")
print(ramped_voltage[volts_val_1[0][int(len(volts_val_1[0])/2)]])
print(e0)
e0=param_list[1][0]
e_std=param_list[1][1]
volts_val_1=np.where((ramped_voltage1>e0-(e_std)) & (ramped_voltage1<e0+(e_std)))
volts_val_2=np.where((ramped_voltage2>e0-(e_std)) & (ramped_voltage2<e0+(e_std)))
plt.axvspan(ramped_time[volts_val_1[0][0]], ramped_time[volts_val_1[0][-1]], color="green", linestyle="--", alpha=0.2)
plt.axvspan(ramped_time[volts_val_2[0][0]+len(ramped_voltage1)], ramped_time[volts_val_2[0][-1]+len(ramped_voltage1)], color="green", linestyle="--", alpha=0.2)
plt.axvline(ramped_time[volts_val_1[0][int(len(volts_val_1[0])/2)]], color="green", linestyle="--", label="Ramped $E_0$")
plt.axvline(ramped_time[volts_val_2[0][int(len(volts_val_2[0])/2)]+len(ramped_voltage1)],color="green", linestyle="--")
plt.legend()
print(e0)
print(ramped_voltage[volts_val_2[0][int(len(volts_val_2[0])/2)]+len(ramped_voltage1)])
#volts_val=np.where(ramped_voltage==param_list[1][0])
