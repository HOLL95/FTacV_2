...
#f=open(filename, "r")
params=["Ru","Cdl","CdlE1", "CdlE2","phase","omega" ,"noise"]
optim_list=["Ru","Cdl","CdlE1", "CdlE2","phase","noise"]
...
for i in range(0, len(concs)):
    for filename in files:#
        if (concs[i] in filename) and (desired_file in filename):
            print(filename, concs[i])

            chains=np.load(("/").join([electrode, folder, filename]))
            print(vals[i])
            plot_params(titles, chains[:, 10000:, :], positions=positions, label=str(vals[i])+"M NaCl")
plt.show()
