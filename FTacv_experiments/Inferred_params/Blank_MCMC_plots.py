...
#f=open(filename, "r")
params=["Ru","Cdl","CdlE1", "CdlE2","phase", "omega"]
optim_list=["Ru","Cdl","CdlE1", "CdlE2","phase", "omega"]
...
extension="high_ru"
desired_file="Noramp_blank_run_1_"
...
for i in range(0, len(concs)):
    for filename in files:#
        print(filename, desired_file+concs[i])
        if (concs[i] in filename) and (extension in filename) and (desired_file in filename):
            print(filename, concs[i])

            chains=np.load(("/").join([electrode, folder, filename]))
            plot_params(titles, chains[:, 15000:, :], positions=positions, label=file_numbers[i])
plt.show()
