...
for lcv_1 in range(0, len(cols)):
    for lcv_2 in range(0, len(rows)):
        #dot_idx=names.index(".")
        filename=("_").join(["Noramp", rows[lcv_2], "cv", cols[lcv_1], "ru"])+".pkl"
        ...
        ax[lcv_2, lcv_1].plot(result.other_values["experiment_voltage"], simulations)
        ax[lcv_2, lcv_1].plot(result.other_values["experiment_voltage"], result.other_values["experiment_current"], alpha=0.7)
        ax[lcv_2, lcv_1].plot(result.other_values["experiment_voltage"], np.subtract(simulations,result.other_values["experiment_current"]))
