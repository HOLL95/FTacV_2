...
struct e_surface_fun {
    ...
    e_surface_fun (
        ...
        E(E),Edc(Edc),dE(dE),cap_E(cap_E),Cdl(Cdl),CdlE(CdlE),CdlE2(CdlE2),CdlE3(CdlE3),E0(E0),Ru(Ru),R(R),k0(k0),alpha(alpha),In0(In0),u1n0(u1n0),dt(dt),gamma(gamma) { }
...
py::object e_surface(double Cdl, double CdlE, double CdlE2, double CdlE3, double omega, double phase,double cap_phase, double pi, double alpha, double E_start, double E_reverse, double delta_E, double Ru, double gamma,double E0, double k0, std::vector<double> times) {
  ...
  if (times.size()==0) {
    ...
    for (int n_out = 0; n_out < times.size(); n_out++) {
        while (t1 < times[n_out]) {
            Itot0 = Itot1;
            double E = c_et( E_start,  E_reverse,  omega,  phase,  v,  delta_E, t1);
            double dE = c_dEdt(E_start,  E_reverse,  omega,  phase,  v,  delta_E, t1+0.5*dt);
            double cap_E=c_et( E_start,  E_reverse,  omega,  cap_phase,  v,  delta_E, t1);
...
PYBIND11_MODULE(isolver_ramped, m) {
