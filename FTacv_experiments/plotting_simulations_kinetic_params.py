...
k_s=np.flip([5**x for x in range(1, 8)])
...
for i in range(0, len(omegas)):
    for j in range(0, len(k_s)):
        ...
        ax[i].plot(voltages[val:], noramp_simulations.i_nondim(1e3*noramp_simulations.test_vals([k_s[j], 8.94], "timeseries")), label="$k^0=$"+str(k_s[j]))
