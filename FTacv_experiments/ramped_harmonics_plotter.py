...
ramp_data_class.simulation_options["dispersion_bins"]=100
...
values=[[0.22292688635059488, 0.050972912676677744, 999.99999739836767, 422.27438618574007, 7.651593539558172e-05, 0.005134821752196414, -0.0005041882668435239, 7.102509666985187e-11, 8.884799587792013, 0, 0.45] ,
[0.23176101084840808, 0.030347974151306224, 107.53613561318956, 890.443764676077, 7.6820398998218e-05, 0.004474624678728965, -0.0004807322727249999, 7.466976561611421e-11, 8.884799587792013, 0, 0.5] ,
[0.23776820129989204, 0.012450970105707577, 69.44127555817347, 869.4862832682389, 7.712489751222832e-05, 0.003922657302229002, -0.0004707909558789216, 7.70666522967133e-11, 8.884799587792013, 0, 0.55] ,
[0.23110722113853555, 0.027345924242786514, 128.71431443672168, 854.1702606093729, 7.77430224842735e-05, 0.0011777514485454693, -0.0003337620915544407, 7.694184659356877e-11,8.884799587792013, 0, 0.6000000000000001] ,
[0.22654888596954947, 0.03836537102264269, 191.6676214677731, 865.687667794221, 7.765938969424711e-05, 0.0012249766508956247, -0.0003337270822888153, 7.478484022832257e-11, 8.884799587792013, 0, 0.65]]
values=[[0.2338340718157331, 0.02098210046833511, 65.12408863808145, 671.0836289401107, 7.639069540336826e-05, 0.005473674200533982, -0.0005412425604750875, 7.59860272754435e-11,8.884799587792013,0],
        [0.22809533506585455, 0.03404445089143265, 110.10937537364292, 699.5264907453902, 7.695998831014857e-05, 0.0031407658207061066, -0.00042313607259804705, 7.477530795339478e-11, 8.884799587792013,0]
]

values=[[0.22809533506585455, 0.03404445089143265, 125.10937537364292, 829.5264907453902, 7.695998831014857e-05, 0.0031407658207061066, -0.00042313607259804705, 7.477530795339478e-11,8.884799587792013,0 ]]
values=[[0.24323038307184336, 0.58906941878404, 55.76171519515906, 872.7221187164696, 7.764498276243975e-05, 0.002120925983914189, -0.000384743271809684, 7.697386442097152e-11, 8.884799587792013,0, 0.5622531162206027]
]
axes=multiplot(1, 1, **{"harmonic_position":0, "num_harmonics":5, "orientation":"landscape", "plot_width":5, "col_spacing":2})
#ramp_fit.simulation_options["alpha_dispersion"]=True
#ramp_fit.param_bounds["alpha"]=[0.58, 0.65]
#alpha_bounds=[[0.5, 0.6],[ 0.55, 0.65]]
...
ramp_fit.simulation_options["dispersion_bins"]=100
axes_keys=sorted(axes.axes_dict.keys())
for q in range(0, len(axes_keys)):
    for i in range(0, len(values)):
        #ramp_fit.param_bounds["alpha"]=alpha_bounds[i]
        ramped_time_series=ramp_fit.i_nondim(ramp_fit.test_vals(values[i+(q*len(values))], "timeseries"))
        ...
        for harm_counter in range(0, len(ramped_harmonics)):
            ax=axes.axes_dict[axes_keys[q]][j%((len(values))*len(ramped_harmonics))]
            ...
            ax2.set_yticks([])
            #if harm_counter==0:
            #    ax.set_title("alpha=U("+str(alpha_bounds[i][0])+", "+str(alpha_bounds[i][1])+")")
