...
param_bounds={
    ...
    'Ru': [0, 100],  #     (uncompensated resistance ohms)
...
test=noramp_fit.test_vals(carbon_means, "timeseries", test=False)
plt.plot(time_results,test)
plt.plot(time_results, current_results)
plt.show()
ramp_free_black=noramp_fit.test_vals(carbon_means, "timeseries", test=False)
...
for i in range(0, num_runs):
    ...
    print found_value
    score_mat[i]=float(found_value)
    param_mat[i,:]=noramp_fit.change_norm_group(found_parameters, "un_norm")
plt.plot(voltage_results, current_results)
for i in range(0, num_runs):
    cmaes_time=noramp_fit.test_vals(cmaes_results, likelihood="timeseries", test=True)
    plt.plot(voltage_results, cmaes_time, label=score_mat[i])
plt.show()
