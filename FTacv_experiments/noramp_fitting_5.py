...
for lcv_2 in range(0, len(concs)):
    for lcv_1 in range(1, 4):
        Method =concs[lcv_2]+"_209_scan"+str(lcv_1)+"_cv"
        type="current"
        type2="voltage"
        ...
        param_list={
            ...
            'E_reverse': ereverse,
            'omega':209,#8.88480830076,  #    (frequency Hz)
            "original_omega":209,
        ...
        param_bounds={
            'E_0':[0.1, 0.6],#[param_list['E_start'],param_list['E_reverse']],
            'omega':[0.95*param_list['omega'],1.05*param_list['omega']],#8.88480830076,  #    (frequency Hz)
            'Ru': [0, 1500],  #     (uncompensated resistance ohms)
        ...
        #expected_params=[0.54, 0.03, 150, 1200, 7e-5, 0, 0, 1e-10, 209, 3*math.pi/2, 3*math.pi/2], 0.5
        for i in range(0, num_runs):
            x0=abs(np.random.rand(len(noramp_fit.optim_list)))#noramp_fit.change_norm_group(expected_params, "norm")
            print(noramp_fit.change_norm_group(x0, "un_norm"))
            cmaes_fitting=pints.OptimisationController(score, x0, sigma0=None, boundaries=CMAES_boundaries, method=pints.CMAES)
            cmaes_fitting.set_max_unchanged_iterations(iterations=200, threshold=1e-9)
            ...
            #plt.plot(noramp_fit.e_nondim(voltage_results), current_results)
            #plt.subplot(1,2,2)
        ...
        filename=("_").join([folder,Method])+".run2"
        ...
        for i in range(0, 10):
            ...
            if rhat_mean<1.2:
                run="MCMC_runs"
                save_file=("_").join([folder,Method, "MCMC"])+".run4"
                filepath=("/").join([dir_path, "Inferred_params", Electrode_save, run])
