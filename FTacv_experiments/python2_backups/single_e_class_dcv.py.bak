...
class single_electron:
    ...
    def define_voltages(self):
        ...
        for i in range(0, len(self.time_vec)):
            voltages[i]=isolver_dcv.c_et(self.nd_param.E_start, self.nd_param.E_reverse,self.nd_param.nd_omega, self.nd_param.phase, self.nd_param.v/abs(self.nd_param.v), self.nd_param.d_E, (self.time_vec[i]))
        return voltages
    ...
    def simulate(self,parameters, frequencies, flag='optimise', flag2='timeseries', test="no"):
        ...
        #print "CdlE1", normed_params[self.optim_list.index("CdlE1")]
