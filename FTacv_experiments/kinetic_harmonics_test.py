...
omegas=[10,50, 100]
k_s=[10, 50, 100, 150, 200, 500, 1000, 2000]
...
for j in range(0, len(k_s)):
    col_idx=int(j%(len(k_s)/2))
    row_idx=int(np.floor(2*j/len(k_s)))
    ...
    for q in range(0, num_scans):
        ...
        param_vals[2]=k_s[j]
        cmaes_time=noramp_results.i_nondim(noramp_results.test_vals(param_vals, method))
        print(noramp_results.dim_dict["k_0"], noramp_results.nd_param.k_0)
        ...
        for z in range(0, num_harms):
            ...
            if z==0:
                ax.set_title(fancy_names["k_0"]+"="+str(k_s[j]))
            if abs(omegas[q])>0.009:
                num_label=str(round(omegas[q], 3))
            else:
                num_label='%.2E' % omegas[q]
            ax2=ax.twinx()
            ax2.set_yticks([])
            ax2.set_ylabel(start_harm+z, rotation=0)
            max_scan=max(scan_harmonics[z])
            min_scan=min(scan_harmonics[z])
            normed_scan=[noramp_results.normalise(x, [min_scan, max_scan]) for x in scan_harmonics[z]]
            ax.plot(voltages, normed_scan, label=num_label+unit_dict["omega"])
            if z<num_harms-1:
                ax.set_xticks([])
            else:
