...
int return_max_idx(vector<double> array1, double max_val){
  int ans=-1;
  cout<<array1[0]<<" "<<array1[1]<<" "<<max_val<<"\n";
...
py::object brent_current_solver(py::dict params, std::vector<double> t, std::string method, double debug=-1, double bounds_val=10) {
    ...
    Itot0 =Cdlp*dE;
    Itot1 = Itot0;
    double error, updated_t;
    int max_idx, min_idx;
    double error_threshold=0.001;
    vector<double> t_intervals{dt, dt/2};
    vector<double> I_n{1000, 1000};
    vector<double> current_vals(3,0);
    double q=0.9;
    ...
    for (int n_out = 0; n_out < t.size(); n_out++) {
      ...
      while(error>error_threshold){
          ...
          while (t1 < t[n_out]) {
                  ...
                  Itot0 = Itot1;
                  if (input==1){
                    E = et(E_start, omega, phase,delta_E ,t1+t_intervals[j]);
                    dE = dEdt(omega, cap_phase,delta_E , t1+0.5*t_intervals[j]);
                    cap_E=et(E_start, omega, cap_phase,delta_E ,t1+t_intervals[j]);
                  }
                  else if (input ==0){
                    E = c_et( E_start, E_reverse, tr, omega,  phase,  v,  delta_E, t1);
                    dE = c_dEdt(tr,  omega,  cap_phase,  v,  delta_E, t1+0.5*t_intervals[j]);
                    cap_E= c_et( E_start, E_reverse, tr, omega,  cap_phase,  v,  delta_E, t1);
                  }
                  else if (input ==2){
                    E=dcv_et(E_start, E_reverse,tr,v, t1);
                    dE=dcv_dEdt(tr,v, t1+0.5*t_intervals[j]);
                    cap_E=E;
                    if(t1>tr){
                      Cdl=-neg_cdl;
                      }
                    }
                  e_surface_fun bc(E,dE,cap_E,Cdl,CdlE,CdlE2,CdlE3,E0,Ru,k0,alpha,Itot0,u1n0,t_intervals[j],gamma);
          ...
          I_n[j]=Itot1;
          max_idx=return_max_idx(t_intervals, *max_element(begin(t_intervals), end(t_intervals)));
          min_idx=abs(max_idx-1);
          error=abs(I_n[max_idx]-I_n[min_idx]);
          updated_t=error_threshold*q*(t_intervals[max_idx]-t_intervals[min_idx])/(error);
          t_intervals[max_idx]=updated_t;
          cout<<t_intervals[0]<<" "<<t_intervals[1]<<"\n";
          if (j==0){
            j=1;
          }else{
              j=0;
