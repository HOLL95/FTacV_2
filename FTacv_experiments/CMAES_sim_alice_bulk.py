...
for exp_num in range(1, 11, 2):
    plot_counter=0
    h_counter=0
    f_counter=0
    figure=multiplot(2, 3, **{"harmonic_position":2, "num_harmonics":7, "orientation":"portrait", "fourier_position":1, "plot_width":5, "row_spacing":2, "plot_height":1, "font_size":15})
    keys=sorted(figure.axes_dict.keys())
    for i in range(exp_num,exp_num+2):
        ...
        figure.axes_dict[keys[0]][plot_counter].text(0.25, 0.95,"Exp "+str(i),
        horizontalalignment='center',
        verticalalignment='center',
        transform = figure.axes_dict[keys[0]][plot_counter].transAxes,
        fontsize=15)
        ...
        #figure.axes_dict[keys[1]][f_counter].legend(loc="center right", bbox_to_anchor=(1, 0.25))
        ...
        for q in range(0, len(data_harms)):
            ...
            if q==len(data_harms)-1:
                ...
                figure.axes_dict[keys[2]][h_counter].legend(loc="lower left",bbox_to_anchor=(-0.35, -1.6))
            #if q==0:
            else:
                figure.axes_dict[keys[2]][h_counter].set_xticks([])
            h_counter+=1
    fig=plt.gcf()
    plt.subplots_adjust(left=0.1, bottom=0.07, right=0.99, top=0.99, wspace=0.41, hspace=0.06)
