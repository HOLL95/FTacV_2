...
for i in range(0, 2):
    ...
    for harm_counter in range(0, len(ramped_harmonics),2):
        plt.subplot(1,2, i+1)
        if i==0:
            current_harm=abs(ramped_data_harmonics[harm_counter,:])
            plt.title("Data")
        else:
            plt.title("Simulation")
            current_harm=abs(ramped_harmonics[harm_counter,:])
        normed_harm=np.zeros(len(current_harm))
        max_harm=max(current_harm)
        min_harm=min(current_harm)
        for j in range(0, len(current_harm)):
            normed_harm[j]=noramp_results.normalise(current_harm[j], [min_harm, max_harm])
        plt.xlabel("Voltage(V)")
        plt.ylabel("Normalised current")
        plt.plot(dcv_plot, normed_harm, label=start_harm+harm_counter)
        """
        ax=axes.axes_dict["row1"][harm_counter]
        #print(j, j%((len(values)//2)*len(ramped_harmonics)))
        ax.plot(ramped_time_results, (ramped_harmonics[harm_counter,:]*1e6), label="Simultation", alpha=0.5)
        #ax.axvline(values[0][0], color="black", linestyle="--")
        #ax.plot(dcv_plot, (ramped_data_harmonics[harm_counter,:]*1e6), label="Data", alpha=0.5)
        ax2=ax.twinx()
        ax2.set_ylabel(harmonic_range[harm_counter], rotation=0)
        ax2.set_yticks([])
        if harm_counter==0:
            ax.set_title("Kinetic dispersion")
