...
py::object e_surface_dispersed(double Cdl, double CdlE, double CdlE2, double CdlE3, double omega, double phase, double pi, double alpha, double E_start, double E_reverse, double delta_E, double Ru, double gamma, std::vector<double> e0_disp, std::vector<double> k0_disp, vector<vector<double>> weight_matrix, std::vector<double> times) {
  //set up temporal mesh
  int bins=e0_disp.size();
  const int v =1;
  const int Nt=200;
  vector<double> Itot;
  double E0;
  ...
  if (times.size()==0) {
    ...
    for (int i=0; i <bins; i++){
      ...
      for (int j=0; j <bins; j++){
        ...
        for (int k = 0; k < Itot.size(); k++){
          Idisp[k]=Idisp[k]+(Itot[k]*weight_matrix[i][j]);
        }
      }
    }
    return py::cast(Idisp);
}
PYBIND11_MODULE(isolver_ramped_dispersion, m) {
	m.def("e_surface_dispersed", &e_surface_dispersed, "solve for I_tot with dispersion");
