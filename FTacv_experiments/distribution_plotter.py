...
f_counter=0
other_files=["6", "9"]
def normal_vals(location, scale, min=None, max=None):
    length=1000
    weights=np.zeros(length)
    if min is None and max is None:
        val_range=np.linspace(norm.ppf(1e-5, loc=location, scale=scale),norm.ppf(1-1e-5, loc=location, scale=scale), length)
    else:
        val_range=np.linspace(min, max, length)

    weights[0]=norm.cdf(val_range[0], loc=location, scale=scale)
    val_range2=np.zeros(len(val_range))
    val_range2[0]=val_range[0]
    for i in range(1, length):
        weights[i]=norm.cdf(val_range[i], loc=location, scale=scale)-norm.cdf(val_range[i-1], loc=location, scale=scale)
        val_range2[i]=(val_range[i]+val_range[i-1])/2
...
for i in range(1, 11):
    ...
    #param_vals=[0.24711362652718327, 0.013408866996237508, 101.33707596282798, 806.5254169541688, 7.743919229112687e-05, 0.0018886700374539794, -0.00033587180721899625, 7.896536259502126e-11, 8.940643685592182, 4.431191900797479, 5.118761570372981, 0.5816941444272093, 0.13997198513370698]
    ...
    e0_weights, e0_vals=normal_vals(noramp_results.dim_dict["E0_mean"], noramp_results.dim_dict["E0_std"],min=0.08, max=0.4)
    alpha_weights, alpha_vals=normal_vals(noramp_results.dim_dict["alpha_mean"], noramp_results.dim_dict["alpha_std"])
    plt.subplot(1,2,1)
    plt.plot(e0_vals[1:], e0_weights[1:], label=str(i))
    plt.xlabel("$E^0$(V)")
    plt.ylabel("Probability density")
    plt.legend()
    plt.subplot(1,2,2)
    plt.plot(alpha_vals[1:], alpha_weights[1:], label=str(i))
    plt.xlabel("$\\alpha$")
    plt.ylabel("Probability density")
    plt.legend()
