...
for lcv_1 in range(2, 11):
    ...
    param_list={
        "E_0":0.2,
        'E_start': -0.35, #(starting dc voltage - V)
        'E_reverse': -0.35+(300e-3*2),
    ...
    vals=[-0.2, 100, 100,1e-4, 0,0, 5.609826904007704e-11,8.94086695405389, 3*math.pi/2, 3*math.pi/2, 0.40000750225452375]
    true_signal=noramp_fit.test_vals(vals, "timeseries")
    test_data=true_signal#noramp_fit.add_noise(true_signal, 0.05*max(true_signal))
