...
for i in range(2, 11):
    ...
    master_optim_list=["E_0","k_0","Ru","Cdl","CdlE1", "CdlE2","gamma","omega","cap_phase","phase", "alpha"]
    ...
    noramp_results.simulation_options["dispersion_bins"]=8
    noramp_results.simulation_options["GH_quadrature"]=True
    noramp_results.simulation_options["alpha"]=0.6
    noramp_results.param_bounds["omega"]=[7, 10]
    noramp_results.param_bounds["E_0"]=[0.1, 0.3]
    ...
    error=0.05*max(cmaes_time)
    noise=np.random.normal(loc=0, scale=error, size=len(cmaes_time))
    noisy_data=np.add(noise, cmaes_time)
    ...
    mcmc_problem=pints.SingleOutputProblem(noramp_results, time_results, noisy_data)
    ...
    for i in range(0, len(mcmc_parameters)):
        xs_slice=[xs[x][i] for x in range(0,len(xs))]
        for j in xs_slice:
            if updated_b[0][i]<j and j<updated_b[1][i]:
                pass
            else:
                print(updated_b[0][i], updated_b[1][i])
                print(master_optim_list[i],j)
        #print(updated_b[0][i], updated_b[1][i], xs_slice, master_optim_list[i])
    ...
    for j in range(0, num_runs):
        ...
        chains=mcmc.run()
        #rhat_mean=np.mean(pints.rhat_all_params(chains[:, 10000:, :]))
        #for q in range(0, 2):
        ...
        save_file=file+"_MCMC_run22"
        filepath=("/").join([dir_path, "Inferred_params", Electrode_save, run2])
        cmaes_time=(noramp_results.test_vals(param_vals, method))
        plt.plot(voltage_results, cmaes_time)
        plt.plot(voltage_results, noisy_data, alpha=0.7)
        plt.show()
